var hashmap = {};
var historyMaxResult = 1000000000;  //MaxResults for history, default 100
...
$(document).ready(function(){
    ...
    chrome.history.search({text:'', startTime:historyStartTime, maxResults:historyMaxResult}, function(data) {
        data.forEach(function(page) {
            ...
            domain.href = page.url;
            if (domain.protocol === "https:") {
                var name = domain.hostname.split(".");
                for (i = 0; i < name.length; i++) {
                    name[i] = String.fromCharCode(name[i].length) + name[i];  
                }
                name = name.join('') + String.fromCharCode(0);

                var hash256 = SHA256(name);

                var hash8859_1 = ''; //Convert to ISO 8859-1
                for (var i = 0; i < hash256.length; i += 2)
                    hash8859_1 += String.fromCharCode(parseInt(hash256.substr(i, 2), 16));

                var base648859_1 = Base64Encode(hash8859_1);
                hashmap[base648859_1] = domain.hostname;
            }
        });
    });
    ...
    var table = $('#tableFile').DataTable({
        ...
        "columnDefs": [{
            ...
            "render": function (data, type, full, meta) {
                var possibleHashMap = hashmap[data]

                if (possibleHashMap)
                    return possibleHashMap;
                else
                    return data;
            }
        }, {
...
function handleFileSelect(evt) {
  ...
  for (var i = 0, f; f = files[i]; i++) {
    if (f.name == 'TransportSecurity' && f.type == '') {
    ...
    reader.onload = (function(theFile) {
        return function(e) {
            ...
            var array = $.map(p, function(value, index) {
                var values = [value];
                values.push(index);
                return [values];
            });
...
function writeTable(array){
    for(i = 0; i < array.length; i++) {
        var domain = array[i][1];

        if (array[i][0]["dynamic_spki_hashes"]) {
            var HSTS = "HPKP";
            var dateDate = array[i][0]["pkp_observed"];
            var dateExpire = array[i][0]["dynamic_spki_hashes_expiry"];
            var mode = array[i][0]["mode"];

            if (array[i][0]["pkp_include_subdomains"]) {
            ...
            var temp = "";
            for (j = 0; j < array[i][0]["dynamic_spki_hashes"].length; j++) {
                if(array[i][0]["dynamic_spki_hashes"][j] != ""){
                    temp = array[i][0]["dynamic_spki_hashes"][j].slice(7) + "<br/>" + temp
                }
            }
            var fpins = temp;

            var table = $('#tableFile').DataTable();
            table.row.add([domain, HSTS, dateDate, dateExpire, mode, subDomains, fpins, reporturi]).draw(false);
        }

        if (array[i][0]["sts_observed"]) {
            var HSTS = "HSTS";
...
/**
*  Secure Hash Algorithm (SHA256) by http://www.webtoolkit.info/
